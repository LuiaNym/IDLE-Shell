...
colors = ["pink", "lightgoldenrod", "palegreen", "hotpink", "darkviolet", "hotpink", "yellowgreen", "aquamarine", "fuchsia"]

# Store circle positions and radii to check overlapping
circles = []

def is_overlapping(x, y, radius):
    """Diğer dairelerle çakışma olup olmadığını kontrol eder."""
    for cx, cy, cr in circles:
        distance = math.sqrt((x - cx)**2 + (y - cy)**2)
        if distance < radius + cr :  ### Overlap Control
            return True
    return False
...
for _ in range(800):
    ...
    max_attempts = 10 ## Number of attempts to place circles without overlapping
    placed = False
